...
class BrushConfig:
    ...
    def get_demo_site_config() -> str:
        desc = (
            "// 以下为配置示例，请参考：https://github.com/kamiilbardaq/MoviePilot-BrushFlow-FreeRemaining/blob/main/plugins.v2/brushflowlowfreq/README.md 进行配置\n"
...
class BrushFlowLowFreq(_PluginBase):
                                                ...
                                                'content': [
                                                    {
                                                        'component': 'VTextField',
                                                        'props': {
                                                            'model': 'free_remaining',
                                                            'label': '最低免费剩余（小时）',
                                                            'placeholder': '如：24；留空不限制',
                                                            'type': 'number',
                                                            'min': '0'
                                                        }
                                                    }
                                ...
                                'content': [
                                    {
                                        ...
                                        'content': [
                                            ...
                                            },
                                            {
                                                'component': 'a',
                                                'props': {
                                                    'href': 'https://github.com/kamiilbardaq/MoviePilot-BrushFlow-FreeRemaining/blob/main/plugins.v2/brushflowlowfreq/README.md',
                                                    'target': '_blank'
                                                },
                                                'content': [
                                                    {
    ...
    def __validate_and_fix_config(self, config: dict = None) -> bool:
        ...
        free_remaining = config.get("free_remaining")
        if free_remaining not in [None, ""] and self.__is_number(free_remaining) \
                and float(free_remaining) < 0:
            self.__log_and_notify_error(f"站点刷流任务出错，最低免费剩余时间不能小于 0：{free_remaining}")
            config["free_remaining"] = None
            found_error = True
    ...
    def __get_free_remaining_hours(freedate: Any, timezone_offset: float = 0) -> Optional[float]:
        ...
        if not freedate:
            return None
        try:
            if isinstance(freedate, datetime):
                free_end = freedate
            else:
                free_end_text = str(freedate).strip()
                if free_end_text.endswith("Z"):
