...
3. Service Worker 校验一次性链接仍位于 M-Team 官方域名。
4. Service Worker 登录 qBittorrent `POST /api/v2/auth/login`。
5. 用 `multipart/form-data` 的 `urls` 字段提交到 `POST /api/v2/torrents/add`，由 qBittorrent 直接获取 torrent，规避浏览器对 M-Team 二进制下载响应的拦截。
6. 使用一次性临时标签轮询 qBittorrent，确认任务真实出现后再向页面提示成功，并自动移除该标签。
7. 若任务已出现但 qB 返回 `error` 状态，页面会直接提示检查保存路径权限。
...
3. 填写 qBittorrent WebUI 地址、用户名和密码。
   - Docker/NAS 映射目录应填写 qBittorrent 容器内部路径；例如容器内 `/downloads/` 映射宿主机 `/vol1/1001/media/downloads/` 时，保存路径填写 `/downloads/`。
...
- M-Team 一次性链接失效，或 qBittorrent 主机访问 M-Team 失败。
- qBittorrent 拒绝新增任务，例如重复种子。
- 扩展升级或重新加载后，旧页面会提示刷新；刷新 M-Team 页面即可重新建立扩展连接。
